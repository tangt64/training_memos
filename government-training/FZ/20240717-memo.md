...
---
1. gogs(8100->80)
2. docker-registry(5000->5000)
3. spring-petclinic(8200->8080)
4. php-ip(8300->80)
5. wordpress+mariadb(80-80, 3306)

업로드가 완료가 되면 "dev-app"라는 POD 생성 후, gogs, docker-registry, spring-petclinic, php-ip를 실행되게 해야 됨. 

```bash
podman pod create --name dev-app --publish 8100:80 --publish 5000:5000 --publish 8200:8080 --publish 8300:80 --publish 80:80

podman pod run --pod pod:dev-app 
```
