...
1. systemd에서 더 이상 fstab를 사용하지 않는다. 블록장치 마운트 시 사용하는 systemd유닛 이름은 무엇인가?

```
(a) .mount (b) .automount (c) .fstab (d) .disk

정답: a
```

2. 파티션 정보 백업을 위해서 사용하는 명령어는 무엇인가?

답: sfdisk

3. crond 및 anacrond를 대신하는 systemd 유닛 이름은 무엇인가?

```
(a) .mount (b) .cron (c) .timer (d) .service

정답: c
```

4. systemd 유닛를 관리하는 명령어는 무엇인가?

답: systemctl 

5. httpd서비스 유닛에서 발생한 오류를 확인하는 명령어는 무엇인가?

답: jouralctl -p err -u httpd

6. 30분 뒤에 date명령어를 실행하려고 한다. 적절한 명령어를 적으시오.

답: systemd-run --on-active="30m"

7. eth1 이더넷 카드를 편집하려고 한다. 올바른 명령어는?

(a) nmtui edit eth1  (b) network-editor eth1  (c) vi ifcfg-eth1

답: a

8. systemd-journald에 중앙 서버를 구축하려고 한다. 다음 중 올바른 소프트웨어 구성은 무엇인가?

(a) systemd-journal-remote, systemd-journal-upload  (b) rsyslog

답: a

9. Rocky Linux 9에서 사용하는 원격 패키지 관리자는 무엇인가?

(a) pip (b) yum (c) apt  (d) dnf

답: d

10. systemd에서 제공하는 NTP서비스는 무엇인가?

(a) ntpd   (b) chronyd  (c) systemd-timesyncd  (d) time-server
