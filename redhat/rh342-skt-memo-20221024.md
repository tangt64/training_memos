...
최국현, 시험보시기 전에 꼭 연락주세요. 추후에 랩이 필요하신 경우에도(시험보기전에).
...
## systemtap, 커널 스페이스 콜 모니터링

커널 모듈로 동작하면서 시스템에서 동작하는 콜 정보를 수집하여 
...
사용이 가능한 기본 커널 모듈은 아래 디렉터리에 있습니다. 
server # cd /usr/share/systemtap/examples/*

커널에서 사용할 모듈을 stap으로 컴파일 합니다.
...
RH342에서 다루지 않지만, 사용하고 싶으신 경우 아래 링크를 참고 하세요.
