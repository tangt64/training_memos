...
- hosts: serverb.lab.example.com
  ...
  - dnf:
    ...
    loop: "{{ packages }}"
    notify: installation is okay

  - debug:
      var: result_packages.rc
  ...
  - systemd:
      ...
      enabled: yes
    loop: "{{ packages }}"
    when: result_packages.rc 
    notify: all services are restarted


handler:
  - name: installation is okay
    debug:
      msg: "the packages are installed"
  - name: all services are restarted
    debug:
      msg: "all services are restarted"      
```
