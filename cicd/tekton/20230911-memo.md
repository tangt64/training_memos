...
```bash
kubectl delete pod --all                                  ## pod제거
kubectl get namespace [ns]

kubectl create ns tekton-lab
kubectl get ns
...
kubectl config set-contexts --current --namespace tekton-lab
kubectl config get-contexts

kubectl run -it --image=centos --rm centos-console /bin/bash  

vi /etc/containers/registries.conf
> unqualified-search-registries = ["registry.fedoraproject.org", "quay.io"]  

kubectl run --image=centos centos-console --dry-run=client --output=yaml > centos-console.yaml     

vi centos-console.yaml
---
apiVersion: v1
kind: Pod
metadata:
...
kubectl apply -f centos-console.yaml
```
