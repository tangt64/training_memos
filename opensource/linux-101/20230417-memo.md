...
systemd resource:
  ...
  - .target(run level)
  - .path 
systemctl: systemd에서 사용하는 서비스 관리 명령어 
...
/run/: runtime
/run/log/journal/:여기에 journald의 데이터베이스 저장
  - journalctl 


rsyslog: 파일, 즉, 텍스트 기반의 로그 파일관리. 관리 도구가 별도로 없음.
journald: 디비, 즉, 바이너리 기반의 로그 파일관리. 관리 도구가 필요 함.  


### 기존과 다른 부분(SystemV, systemD)

Unit: systemd에서 사용하는 자원 설정 파일. 이 설정 형식이 INI(TOML)형태로 되어있음. 
  - /usr/lib/systemd/system의 모든 파일들이 유닛

.service: 일반적으로 사용하는 웹 같은 서비스. systemd의 유닛중에 하나. 
...
yum -y --releasever=9 --nogpg --installroot=/srv/test install systemd passwd yum centos-release vim-minimal
setenforce 0
...
리눅스에서 사용하는 옵션에 Z라고 붙이면, 보통은 자원이 사용중인 컨텍스트가 출력됨.
컨텍스트 관리를 위해서 사용하는 명령어는 "semanage"이다. 
