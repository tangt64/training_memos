...
## 다음 사용자를 생성한다.
  - 사용자 test1는 shared에 포함이 된다.
  - tim은 쉘 사용이 불가능하다.
  - 사용자 test2는 쉘 tcsh를 사용한다.
  - 사용자 test3은 shared에 포함이 된다.
  - 사용자 test4는 shared에 포함이 되며, 쉘은 fish를 사용한다.
  - 사용자 test5, test6은 홈 디레터리가 없다.
...
## 시간 서버 설정을 아래와 같이 변경한다.
  - time.google.com
  - lab1, lab2 둘 다 동일한 주소를 사용한다.
...
## 문자열/복사/검색/압축
  ...
  - 문자열 GNU를 찾는다.
    + 대소문자 구별없이 GNU문자열이 있으면 /root/gnulist.txt파일에 내용 및 파일 이름이 같이 저장되도록 한다.

## 블록
  - 500메가 스왑을 생성한다.
    + 반드시 부팅이 된 이후에도 올바르게 동작이 되어야 한다.
  - 800메가 스왑을 생성한다.
    + 해당 스왑은 vgswap에 lvswap에 구성한다. 
    + 반드시 부팅이 된 이후에도 올바르게 동작이 되어야 한다.
  - 2기가 LVM디스크를 생성한다.
    + 해당 디스크는 2gvg에 2glv라는 논리적 디스크 이름으로 구성이 된다.
