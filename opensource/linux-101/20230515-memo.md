...
bootloader --> 방향키로 인트럽트 --> e --> linux으로 커서 이동 --> rd.break --> ctrl + x --> shell --> mount | grep sysroot(ro/ReadOnly) --> mount -oremount,rw /sysroot --> mount | grep sysroot(rw/ReadWrite) --> echo rocky | passwd --stdin root --> vi /etc/fstab --> touch /.autorelabel, SELinux 확장 속성 재구성 --> exit --> exit(reboot)
```
