...
nmcli connection add ipv4.addresses 192.168.10.110/24 ipv4.dns 8.8.8.8 ipv4.gateway 0.0.0.0 ipv4.method manual type ethernet ifname eth1 con-name eth1-static
nmtui hostname
nmtui edit eth1
...
kubeadm init --apiserver-advertise-address 192.168.90.110 --pod-network-cidr=192.168.0.0/16 --upload-certs
```



1. 네트워크 카드 설정
2. 모듈(드라이버)영구적으로 적용 및 등록(램 디스크 갱신)
3. 커널 파라메터 영구적으로 적용 및 등록(램 디스크 갱신)
4. 저장소 미러링 + 디스크 관리
