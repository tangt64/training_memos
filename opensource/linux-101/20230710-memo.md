...
RHEL 7/8: NetworkManager, network-scritps
RHEL 9: network-scripts(deprecated), NetworkManager 혹은 systemd기반
...
__nmcli:__ CLI 명령어 도구, 스크립팅이나 혹은 자동화시 사용.
  - 프로파일 및 모든 네트워크 장치 구성이 가능.

__nmtui:__ 텍스트 기반 인터페이스 도구.
  - 프로파일 및 장치 구성이 가능.
  - 호스트 이름 설정도 가능.

__nm-connection-editor:__ GUI 기반 인터페이스 도구
...
__rescue:__ OS영역까지 램 디스크에 마운트가 된 상태.(rw)

__emergency:__ OS영역에 대해서 마운트가 되어 있지 않는 상태.(ro) 혹은 아에 구성이 안되어 있는 경우도 있음. remount, chroot
...
-atime: 사용자가 접근한 access 시간.
-size: Unit단위로 크기 명시.
-type: 파일(f), 디렉터리(d) 조건 검색(형식).
-exec: find에서 찾은 결과를 특정 명령어로 전달 및 실행
