...
1. windows 서버 1개, 리눅스 서버 1개(public network)
  1-1. windows서버는 내부 서비스 테스트 용도 데스크탑
  1-2. windows서버(클라이언트)는 example.com같은 도메인에 접근 용도
  1-3. 외부에서 접근이 가능한 bastion server(linux, windows)
2. 두 대의 리눅스 서버를 구성 및 생성(private network)
  2-1. mysql(mariadb-server)구성 ---> test-db-tang
  2-2. web(httpd, apache)구성    ---> test-web-tang
  2-3. wordpress                ---> test-web-tang
  2-4. 둘 다 리눅스 서버 기반으로 구성
