...
다음과 같이 앤서블 프로젝트 디렉터리를 생성 후, ansible 설정을 적용한다.

1. 설정 파일은 ansiblelab에 생성한다.
2. 인벤토리 파일 이름은 hosts로 한다.
3. roles 경로 ansiblelab/roles를 추가한다.
4. 외부에 접근 시 사용하는 계정은 user1으로 설정한다.
