...
ansible all -b -m yum_repository -a 'name=internal description=hehe file=internal.repo baseurl=http://control.example.com gpgcheck=0'

vi repos.sh
> #!/bin/bash
> ansible all -b -m copy -a src="repos/rocky.repo" -a dest="/etc/yum.repos.d/"
> ansible all -b -m yum_repository -a 'name=internal description=hehe file=internal baseurl=http://control.example.com gpgcheck=0' 
chmod o+x repos.sh
./repos.sh
