...
for i in node{1..3} ; do sshpass -p rocky ssh root@$i 'dnf install firewalld && systemctl enable --now firewalld' ; done
> dnf install firewalld && systemctl enable --now firewalld
for i in {1..3} ; do sshpass -p rocky ssh root@node${i} 'firewall-cmd --add-service=high-availability && firewall-cmd --runtime-to-permanent' ; done
for i in node{1..3} ; do sshpass -p rocky ssh root@$i 'systemctl stop firewalld' ; done
> systemctl stop firewalld   ## 사용하지 않으시면


for i in {1..3} ; do sshpass -p rocky ssh root@node$i 'echo centos | passwd --stdin hacluster && systemctl enable --now pcsd.service' ; done
