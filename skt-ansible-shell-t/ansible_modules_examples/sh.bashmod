...
function create_file
    ...
    contents=$(cat "$dest" 2>&1 | python2 -c 'import json,sys; print json.dumps(sys.stdin.read())')
    }
...
function delete_file
    ...
    if [ -f "$dest" ]; then
        ...
        msg="file deleted"
        contents=$(cat "$dest" 2>&1 | python2 -c 'import json,sys; print json.dumps(sys.stdin.read())')
        output=$(rm -f $dest 2>&1 | python2 -c 'import json,sys; print json.dumps(sys.stdin.read())')
        if [ $? -ne 0 ]; then
...
function convert_to_upper
    ...
    if [ "$current" = "$new" ]; then
        ...
        contents=$(printf "$current" | python2 -c 'import json,sys; print json.dumps(sys.stdin.read())')
    else

        changed="true"
        msg="${msg}file converted to upper case"
        contents=$(printf "$new" | python2 -c 'import json,sys; print json.dumps(sys.stdin.read())')
    fi
    }
...
function convert_to_lower
    ...
    contents=$(ls -l "$dest" 2>&1 | python2 -c 'import json,sys; print json.dumps(sys.stdin.read())')
    current=$(cat $dest)
